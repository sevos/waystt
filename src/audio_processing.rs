...
impl AudioProcessor {
    ...
    pub fn trim_silence(&self, samples: &[f32]) -> Result<Vec<f32>> {
        if samples.is_empty() {
            return Err(anyhow::anyhow!("Cannot trim silence from empty audio buffer"));
        }
...
mod tests {
    ...
    fn test_trim_silence_empty_buffer() {
        ...
        assert!(result.unwrap_err().to_string().contains("empty audio buffer"));
    }
    ...
    fn test_validate_audio_no_signal() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("no detectable signal"));
    }
    ...
    fn test_process_for_speech_recognition_valid() {
        ...
        let peak = processed.iter().map(|&s| s.abs()).fold(0.0f32, |acc, x| acc.max(x));
